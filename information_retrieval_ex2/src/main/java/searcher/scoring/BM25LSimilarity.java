...
public class BM25LSimilarity extends Similarity {
  ...
  public final SimWeight computeWeight(float queryBoost, CollectionStatistics collectionStats, TermStatistics... termStats) {
    ...
    for (int i = 0; i < cache.length; i++) {
      cache[i] =  ((1 - b) + b * decodeNormValue((byte)i) / avgdl); //deleted k1*
    }
  ...
  private class BM25DocScorer extends SimScorer {
    ...
    public float score(int doc, float freq) {
	  ...
	  if(norms==null)
	  {
		return weightValue * freq / (freq + k1);
	  }
	  else
	  {
  ...
  private Explanation explainScore(int doc, Explanation freq, BM25Stats stats, NumericDocValues norms) {
    ...
    if (norms == null) {
      ...
      tfNormExpl.setValue((((freq.getValue()/(1 - b + b * doclen/stats.avgdl))+d) * (k1 + 1)) / ( k1 +(freq.getValue()/(1 - b + b * doclen/stats.avgdl))+d));
      if((freq.getValue()/(1 - b + b * doclen/stats.avgdl))<=0)
